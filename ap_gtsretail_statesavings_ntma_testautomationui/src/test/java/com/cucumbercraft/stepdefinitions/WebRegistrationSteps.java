...
//            matchDetials.selectNumberPrefix();
    ...
    @Then("^Enter all address details$")
    public void enterAllAddressDetails() {
        try {
            matchDetials.selectCountry1();
            matchDetials.enterAddressLine11();
            matchDetials.enterAddressLine22();
            matchDetials.enterTown1();
            matchDetials.selectCountry_Region_State1();
            matchDetials.enterEirCode1();
            matchDetials.enterEmailAddress1();
            // matchDetials.selectNumberPrefix1();
            matchDetials.enterNumber1();
        } catch (Exception e) {
            Assert.fail(e.getMessage());
        }
    }

    @And("^click on send code of confirm your mobile page$")
    public void clickOnSendCodeOfConfirmYourMobilePage() {
        try {
            confirmMobile.clickSendCode();
        } catch (Exception e) {
            Assert.fail(e.getMessage());
        }
    }

    @And("^Enter the security code web$")
    public void enterTheSecurityCodeweb() {
        try {
            securityCode.enterSecuritycodeforweb(regstrnData.get("otpNumber1"));
        } catch (Exception e) {
            Assert.fail(e.getMessage());
        }
    }
    ...
    @Then("Validate error message displayed for all {string} on We Matched your details page")
    public void validateErrorMesageDisplayedForAllFeildsOnWeMatchedYourDetailsPage(String feildType) {
        matchDetials.validateErrorMessage(feildType);
    }

    @Then("Validate error message displayed when consent checkbox not selected")
    public void validateErrorMessageDisplayedWhenConsentCheckboxNotSelected() {
        checkDetails.validateConsentError();


    }

    @Then("Validate error message on Confirm your Mobile Number page when otp is: {string}")
    public void validateErrorMessageOnConfirmYourMobileNumberPageWhenOtpIs(String feildType) {
        securityCode.validateOTPErrorMessage(feildType);
    }
