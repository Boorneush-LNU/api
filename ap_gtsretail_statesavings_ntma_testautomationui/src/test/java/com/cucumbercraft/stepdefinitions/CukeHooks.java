...
//        ExtentCucumberAdapter.getCurrentScenario().info(scenario.getName()); // SSG: check after final implementation
...
//        RunCucumberTests.statusMap.add(Map.of(testName, ExtentCucumberAdapter.getCurrentScenario().getStatus().getName()));
        ...
        DriverManager.removeChromeOptions();
        Settings.removeInstance();
        // Release per-thread Excel HashMap data to allow GC to reclaim memory
        // between parallel test executions (prevents OutOfMemoryError).
