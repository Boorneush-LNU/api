...
public class WebDriverFactory {
    ...
    public static WebDriver getWebDriver(Browser browser) {
        ...
        try {
            switch (browser) {
                case CHROME:
                    ChromeOptions options = buildChromeOptions(properties, DriverManager.getTestParameters());
                    DriverManager.setChromeOptions(options);          // ← store in ThreadLocal
                    driver = new ChromeDriver(DriverManager.getChromeOptions());
                    break;
...
//        desiredCapabilities.setJavascriptEnabled(true); // Pre-requisite for
    ...
    private static ChromeOptions buildChromeOptions(Properties properties, SeleniumTestParameters testParameters) {
        ...
        String downloadInd = testParameters.getEnableDownloadInd().getValue();
        if (downloadInd.equalsIgnoreCase("Yes")) {
            prefs.put("download.default_directory", Util.getTargetPath());
            prefs.put("plugins.always_open_pdf_externally", true);
        } else if (downloadInd.equalsIgnoreCase("NA")) {
            prefs.put("download.default_directory",
                    Util.getTargetPath() + Util.getFileSeparator() + "AddHoldings");
            prefs.put("plugins.always_open_pdf_externally", true);
        }
