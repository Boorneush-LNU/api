...
//                    driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
    ...
    public void gettextlog(By locator, BiPredicate<String, String> comparisonMethod, String expectedResult, String strType) {
        ...
        try {
            ...
            String color = condition ? "yellow" : "red";
            String resultText = condition ? "PASS" : "FAIL";
            String message = condition ? String.format(strType+" : <font color='%s'><b>%s</b></font> : contains the expected text : <font color='%s'><b>%s</b></font>", color, actualResult, color, resultText) :
                    String.format(strType+" : <font color='%s'><b>%s</b></font> : does not match the expected text : <font color='%s'><b>%s</b></font> : <font color='%s'><b>%s</b></font>", color, actualResult, color, expectedResult, color, resultText);

            if (condition) {
                ExtentCucumberAdapter.getCurrentStep().pass(message);
                Allure.step(message, io.qameta.allure.model.Status.PASSED);
                System.out.println(strType+" >> Actual-" + actualResult + " >> Expected -" + expectedResult);

            } else {
                ExtentCucumberAdapter.getCurrentStep().fail(message);
                Allure.step(message, io.qameta.allure.model.Status.FAILED);
                System.err.println(strType+" >> Actual-" + actualResult + " >> Expected -" + expectedResult);
...
//                        currentScenario.embed(Util.takeScreenshot(driver), "image/png");
    ...
    public boolean CompareString(String actualResult, BiPredicate<String, String> comparisonMethod, String expectedResult) {
        try {
            ...
            if (condition) {
                ...
                System.err.println(">> Actual-" + actualResult + " >> Expected -" + expectedResult);
                log.error(">> Actual-" + actualResult + " >> Expected -" + expectedResult);
                System.out.println("Difference: "+StringUtils.difference(actualResult,expectedResult));
    ...
    public void CompareString(String actualResult, BiPredicate<String, String> comparisonMethod, String expectedResult, String strType) {


        boolean condition = comparisonMethod.test(actualResult, expectedResult);
        String color = condition ? "yellow" : "red";
        String resultText = condition ? "PASS" : "FAIL";
        String message = condition ? String.format(strType + ": <font color='%s'><b>%s</b></font> : contains the expected : <font color='%s'><b>%s</b></font>", color, actualResult, color, resultText) :
                String.format(strType + ": <font color='%s'><b>%s</b></font> : does not match the expected : <font color='%s'><b>%s</b></font> : <font color='%s'><b>%s</b></font>", color, actualResult, color, expectedResult, color, resultText);

        if (condition) {
            FrameworkLogger.log(LogType.EXTENT_AND_CONSOLE_PASS,message);
            System.out.println(">> Actual-" + actualResult + " >> Expected -" + expectedResult);

        } else {
            FrameworkLogger.log(LogType.EXTENT_AND_CONSOLE_FAIL,message);
            System.err.println(">> Actual-" + actualResult + " >> Expected -" + expectedResult);
            System.out.println("Difference between Actual & Expected: "+ StringUtils.difference(actualResult,expectedResult));
...
//        driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
    ...
    public boolean isElementDisplayed(WebElement locator, int timeout) throws InterruptedException {
        boolean elementPresent = true;
        int count = 0;
...
//		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
    ...
    public boolean isElementDisplayed(By locator) throws InterruptedException {
        return isElementDisplayed(locator, DEFAULT_TIMEOUT);
    }

    public boolean isElementDisplayed(WebElement elemVar) throws InterruptedException {
        return isElementDisplayed(elemVar, DEFAULT_TIMEOUT);
    }
...
//            scroll(element);
    ...
    public void sendKeysEnter(By locator, String valuetoType) {

        try {
            WebElement element = waitUntilElementVisible(locator, 10);
//            scroll(element);
            element.clear();
            element.sendKeys(valuetoType,Keys.TAB);
        } catch (StaleElementReferenceException e) {
            driver.findElement(locator).clear();
            driver.findElement(locator).sendKeys(valuetoType,Keys.ENTER);
        } catch (TimeoutException e) {
    ...
    public void click(By locator) {
        ...
        try {
            waitUntilElementToBeClickable(locator, 7).click();
...
//		wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.id("eFormContents")));
    ...
    public boolean objIsEnabled(By locator) {
    ...
    public void sendKeysLog(By locator, String valuetoType, String Log) throws InterruptedException {
        try {

            if (isElementDisplayed(locator, 1000)) {
                driver.findElement(locator).clear();
                driver.findElement(locator).click();
                driver.findElement(locator).sendKeys(valuetoType);
                ExtentCucumberAdapter.addTestStepLog("Value entered in  :" + Log + " </b>:Pass");
            } else {
                ExtentCucumberAdapter.addTestStepLog(Log + "<b>Object not Found</b>:Fail");
            }
        } catch (StaleElementReferenceException e) {
            ExtentCucumberAdapter.addTestStepLog("Exception  Found </b>:Fail " + e);
//			currentScenario.embed(Util.takeScreenshot(driver), "image/png");
            currentScenario.attach(Util.takeScreenshot(driver), "image/png", currentScenario.getName());
            Assert.fail();
        }
    ...
    public void navigateBackFunc()  {

        if(driver.getTitle().equalsIgnoreCase("504 Gateway Timeout")){
            driver.navigate().refresh();
        }else if(driver.getTitle().equalsIgnoreCase("Server Hangup")){
            driver.navigate().refresh();
        }else{
    ...
    public BiFunction<String, Integer, By> getXPathWithIndex = (xpathTemplate, index) -> By.xpath(String.format(xpathTemplate, index));



    public void gettextByAttribute(By locator, BiPredicate<String, String> comparisonMethod, String expectedResult, String Attribute) {

        String actualResult = "";

        try {
            actualResult = waitUntilElementLocated(locator, 15).getAttribute(Attribute);
        ...
        String color = condition ? "yellow" : "red";
        String resultText = condition ? "PASS" : "FAIL";
        String message = condition ? String.format("Attribute text: <font color='%s'><b>%s</b></font> : contains the expected text : <font color='%s'><b>%s</b></font>", color, actualResult, color, resultText) :
                String.format("Attribute text: <font color='%s'><b>%s</b></font> : does not match the expected text : <font color='%s'><b>%s</b></font> : <font color='%s'><b>%s</b></font>", color, actualResult, color, expectedResult, color, resultText);
    ...
    public void gettextByAttribute(WebElement webElement, BiPredicate<String, String> comparisonMethod, String expectedResult, String Attribute) {

        String actualResult = "";

        try {
            actualResult = waitUntilElementVisible(webElement, 15).getAttribute(Attribute);
        } catch (Exception ignored) {}
    ...
    public void openLinkInNewTab(By by, String expectedTitle) {
        ...
        if (driver.getWindowHandles().size() == 2) {
            ...
            if (actualTitle.equals(expectedTitle))
                ExtentCucumberAdapter.getCurrentStep().pass("Title matching with expected: <font color='yellow'><b>" + expectedTitle + "</b></font>");
            else
                ExtentCucumberAdapter.getCurrentStep().fail("Title Actual :<font color='red'><b>" + actualTitle + "</b></font> not matching Expected : <font color='red'><b>" + expectedTitle + "</b></font>");
            driver.close();
            driver.switchTo().window(driver.getWindowHandles().stream().findFirst().get());
        }else {
            ExtentCucumberAdapter.getCurrentStep().fail("<font color='red'>External link opened in same tab</font>");
            waitForPageLoaded();
            String actualTitle = driver.getTitle();
            if (actualTitle.equals(expectedTitle))
                ExtentCucumberAdapter.getCurrentStep().pass("Title matching with expected: <font color='yellow'><b>" + expectedTitle + "</b></font>");
    ...
    public void openPDFInNewTab(By by, String expectedTitle) {
        clickLog(by,expectedTitle);
        var size=driver.getWindowHandles().size();
        ...
        if (driver.getWindowHandles().size() == 2) {
           ...
           String actualTitle = driver.getCurrentUrl();
            ...
            if (actualTitle.contains(expectedTitle))
                ExtentCucumberAdapter.getCurrentStep().pass("Url conatins  expected: <font color='yellow'><b>" + expectedTitle + "</b></font>");
            else
                ExtentCucumberAdapter.getCurrentStep().fail("URL Actual :<font color='red'><b>" + actualTitle + "</b></font> not matching Expected : <font color='red'><b>" + expectedTitle + "</b></font>");
            driver.close();
            driver.switchTo().window(driver.getWindowHandles().stream().findFirst().get());
        }else {
            ExtentCucumberAdapter.getCurrentStep().fail("<font color='red'>PDF opened in same tab</font>");
            String actualTitle = driver.getCurrentUrl();
            if (actualTitle.contains(expectedTitle))
                ExtentCucumberAdapter.getCurrentStep().pass("Url conatins  expected: <font color='yellow'><b>" + expectedTitle + "</b></font>");
            else
    ...
    public void HoverOnElement(By by, int timeout ) {
    ...
    public WebElement waitUntilElementToBeClickable(WebElement webElement, long timeOutInSeconds) {
        return new WebDriverWait(driver, Duration.ofSeconds(timeOutInSeconds)).until(ExpectedConditions.elementToBeClickable(webElement));
    }

    public void clickLog(WebElement locator, String Log) {
    ...
    public void gettextByAttributeValidator(By locator, String comparetype, String ExpectedResult,String Attribute, String messageLog) {
        ...
        try {
            ...
            switch (comparetype) {
                ...
                case "EqualsIgnoreCase":
                    if (ActualResult.equalsIgnoreCase(ExpectedResult)) {
                        ExtentCucumberAdapter.getCurrentStep().pass(messageLog+"<b> The Text : " + ActualResult + "</b> : matches with expected text : <b>PASS </b>");
                    } else {
                        ExtentCucumberAdapter.getCurrentStep().fail(messageLog+" <p style='color:red'> <b>  The Text : " + ActualResult + "</b> :  does not matches with expected <b> "+ExpectedResult+" : FAIL </b>");
                ...
                case "contains": {
                    if (ActualResult.contains(ExpectedResult)) {
                        ExtentCucumberAdapter.getCurrentStep().pass(messageLog+"<b> The Text :" + ActualResult + "</b> : contains the expected text : <b>PASS </b>");
                    } else {
                        ExtentCucumberAdapter.getCurrentStep().fail(messageLog+"<p style='color:red'> <b> The Text :" + ActualResult + "</b> :  contains the not expected <b> "+ExpectedResult+" :FAIL </b>");
                        System.out.println(">>"+ActualResult+" "+ExpectedResult);
                        //currentScenario.embed(Util.takeScreenshot(driver), "image/png");
                        currentScenario.attach(Util.takeScreenshot(driver), "image/png",currentScenario.getName());
                    }
            ...
            ExtentCucumberAdapter.getCurrentStep().fail("Exception  Found </b>:Fail " + ex);
//			currentScenario.embed(Util.takeScreenshot(driver), "image/png");
            currentScenario.attach(Util.takeScreenshot(driver), "image/png",currentScenario.getName());
            Assert.fail("Failed "+ex.getMessage());
        }
    ...
    public void gettextByAttributeValidator(WebElement elemVar, String comparetype, String ExpectedResult,String Attribute, String messageLog) {

        String ActualResult = "";
        try {
            if (isElementDisplayed(elemVar, 5)) {
                ActualResult = elemVar.getAttribute(Attribute);
            } else {
                ActualResult = "";
            }
            switch (comparetype) {
...
//			currentScenario.embed(Util.takeScreenshot(driver), "image/png");
    ...
    public void clickLogW(By locator, String Log) {
        try {
            WebElement element=waitUntilElementToBeClickable(locator,10);
            scroll(element);
            element.click();
            waitFor(4000);
            if (!Log.isEmpty()) {
    ...
    public boolean CompareStringStr(String ActualResult, String comparetype, String ExpectedResult)
    {
        try{
            switch (comparetype) {
                case "equalsIgnoreCase":
                case "EqualsIgnoreCase":
                    if (ActualResult.equalsIgnoreCase(ExpectedResult)) {
                        ExtentCucumberAdapter.addTestStepLog("<b> The Text : " + ActualResult + "</b> : matches with expected text : <b>PASS </b>");
                    } else {
                        ExtentCucumberAdapter.addTestStepLog("<p style='color:red;'> <b>  The Text : " + ActualResult + "</b> :  does not matches with expected <b> "+ExpectedResult+" : FAIL </b>");
                        System.err.println(">>"+ActualResult+" "+ExpectedResult);
                        //currentScenario.embed(Util.takeScreenshot(driver), "image/png");
                        currentScenario.attach(Util.takeScreenshot(driver), "image/png",currentScenario.getName());
                    }
                ...
                case "contains": {
                    if (ActualResult.contains(ExpectedResult)) {
                        ExtentCucumberAdapter.addTestStepLog("<b> The Text :" + ActualResult + "</b> : contains the expected text : <b>PASS </b>");
                    } else {
                        ExtentCucumberAdapter.addTestStepLog("<p style='color:red;'> <b> The Text :" + ActualResult + "</b> :  contains the not expected <b> "+ExpectedResult+" :FAIL </b>");
                        System.err.println(">>"+ActualResult+" "+ExpectedResult);
                        //currentScenario.embed(Util.takeScreenshot(driver), "image/png");
    ...
    public void CompareStringStr(String actualResult, BiPredicate<String, String> comparisonMethod, String expectedResult) {
        try {
            ...
            String color = condition ? "yellow" : "red";
            String resultText = condition ? "PASS" : "FAIL";
            String message = condition ?String.format("The Text: <font color='%s'><span>%s</span></font> : contains the expected text : <font color='%s'><span>%s</span></font>", color, actualResult, color, resultText):
