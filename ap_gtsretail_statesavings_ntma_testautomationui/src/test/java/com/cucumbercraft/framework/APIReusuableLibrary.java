...
public class APIReusuableLibrary {
    ...
    public Map<String, String> getUserDetails(String username) throws FileNotFoundException {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Response response = RestAssured.given()
                    ...
                    .headers(headerMap)
                    .baseUri(FrameworkConstants.baseUriMyAccountFEAPILinkFetch())
                    .queryParam("username", username)
                    .when().log().all()
                    .get("api/v5/account/get-user-details").then().log().all().extract().response();
    ...
    public Response sendGetRequestWithOptionalParamsFunctional(String endpoint, Map<String, Object> queryParams, Map<String, String> header) {
        RequestSpecification request = RestAssured.given().relaxedHTTPSValidation().headers(header);

        queryParams.forEach((key, value) -> {
            Optional.ofNullable(value).ifPresent(val -> request.queryParam(key, val));
        });

        return request.get(endpoint);
    }

    public Response fetchUnreadNotificationCountRefFunc(String queryParamValue, String queryParamkey,Map<String, String> headerMap, Map<String, String> queryMap, String baseUri, String apiEndpointName) {
        ...
        Response responseTemp=RestAssured.given()
                ...
                .when()
                .get(apiEndpointName).then().extract().response();
