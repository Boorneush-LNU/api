...
public class SelectProduct {
    ...
    private final By changeIbancloseslider = By.xpath("//section[@id='sectionConfirmChangeIban']//button[@class='modal_close js-closeModal']");
    private final String productTiles = "//div[@class='product-review-list--row row']";
    private final By changeIbanverificationcloseslider = By.xpath("//section[@id='sectionVerifyCode']//button[@class='modal_close js-closeModal js-one-time-code-close']");
    private final String productTileAmount = "//div[@class='product-review-list--row row']/div[2]";
//    private final By allocatefullamount = By.id("btnAllocateFull");
    private final String allocatefullamount = "(//div[@name='productCard'])[%s]/descendant::button[text()='Allocate full amount'][%s]";
    ...
    private final By addIbancloseslider = By.xpath("//button[@class='modal_close js-closeModal' and @aria-label='Close modal' and @onclick='modals.CloseModal()']");
...
//    private final By confirmbttn = By.id("submitButton"); .cta-container button:nth-of-type(2)
    private final By confirmbttn =  By.xpath("//button[@class='button button--primary gtm-cta']");
    ...
    private final By cancelbutton = By.xpath("//button[@class='gtm-cta button button--secondary js-closeModal' and @onclick='modals.closeModal()'  and text()='Cancel']");
    private final By cancelbtnIban = By.xpath("//button[@class='gtm-cta button button--secondary js-closeModal' and @onclick='modals.closeModal()'  and text()='Cancel']");
    ...
    private final By chgIbandidnotreceiveclosebtn = By.xpath("//section[@id='sectionResendSecurityCode']//button[@class='modal_close js-closeModal js-resend-security-code-close']");
    private final By changeIbanverificationcancelslider = By.xpath("//button[@onclick='CancelResend()']");
    Modals modal;
    ...
    public void selectProductXpath(int i, String product) throws Exception {
        ...
        if (product.contentEquals("Deposit Account (POSB - book based)")) {
            ...
            if (!data.getPosbAccountNumber().contentEquals("null")) {
                webUtil.click(checkboxdeposit);
                webUtil.waitForPageLoaded();
                webUtil.waitUntilElementLocated(acno, 10);
                webUtil.sendKeys(acno, data.getPosbAccountNumber());
            }
        } else {
            try {
                webUtil.selectDropDown(By.xpath(String.format(chsprdct, i)), select -> select.selectByVisibleText(product));
    ...
    public void chgIbanVerificationCloseBtn() throws Exception {
        if (webUtil.isElementclickable(changeIbanverificationcloseslider, 20)) {
            webUtil.click(changeIbanverificationcloseslider);
        } else {
            throw new ExceptionUtils("Close button x-path changed");
        }
    }

    public void chgIbanNotReceivedCancelBtn() throws Exception {
        if (webUtil.isElementclickable(changeIbanverificationcancelslider, 20)) {
            webUtil.click(changeIbanverificationcancelslider);
        } else {
            throw new ExceptionUtils("Close button x-path changed");
        }
    }

    public void chgIbanNotReceivedCloseBtn() throws Exception {
        if (webUtil.isElementclickable(chgIbandidnotreceiveclosebtn, 20)) {
            webUtil.click(chgIbandidnotreceiveclosebtn);
        } else {
            throw new ExceptionUtils("Close button x-path changed");
        }
    }

    public void changeIbanCloseBtn() throws Exception {
        if (webUtil.isElementclickable(changeIbancloseslider, 20)) {
            webUtil.click(changeIbancloseslider);
        } else {
            throw new ExceptionUtils("Close button x-path changed");
        }
    }
    ...
    private void chooseMultipleProduct(List<String> list, List<String> amount1)  {
        ...
        for (int i = 0; i < list.size(); i++) {
            try {
                if (webUtil.isElementDisplayed(By.xpath(String.format(chsprdct, i + 1)), 05)) {
                    selectProductXpath(i, list.get(i));
                    userEnterAnAmount(amount1.get(i), list.get(i), i);
                    if ((i + 1) < list.size()) {
    ...
    private void chooseSingleProduct1(String productList) throws Exception {
        maturityvalue = Double.parseDouble(webUtil.getText(totalamt).replaceAll("[^0-9.]", ""));
        try {
            selectProductXpath(0, productList);
        } catch (Throwable throwable) {
    ...
    public void userClicksOnDropdownMenuAndSelect(List<String> productList, List<String> amount) throws Exception {

        if (productList.size()>1) {
            chooseMultipleProduct(productList, amount);
        } else {
            chooseSingleProduct(productList.get(0), amount.get(0));
        }
        ExtentCucumberAdapter.addTestStepScreenCaptureFromPath(Util.takeScreenshotasFile(driver));
    }


//    Add this for failed TC
    public void userClicksOnDropdownMenuAndSelectSingle(List<String> productList, List<String> amount) throws Exception {

        if (productList.size()>1) {
            chooseMultipleProduct(productList, amount);
        } else {
            chooseSingleProduct1(productList.get(0));
        }
        ExtentCucumberAdapter.addTestStepScreenCaptureFromPath(Util.takeScreenshotasFile(driver));
    }

    //Shan 29.7.25
    public void userClicksOnDropdownMenuAndSelect2(List<String> productList, List<String> amount) throws Exception {
            chooseSingleProduct(productList.get(0), amount.get(0)); // Select 1st Product & amount
        ExtentCucumberAdapter.addTestStepScreenCaptureFromPath(Util.takeScreenshotasFile(driver));
    }

    //Shan 29.7.25
    public void userClicksOnDropdownMenuAndSelectSingle(List<String> productList) throws Exception {
        chooseSingleProduct1(productList.get(1));  // Select the 2nd Product from the Cell after Edit clicked
        ExtentCucumberAdapter.addTestStepScreenCaptureFromPath(Util.takeScreenshotasFile(driver));
    }

    public void userClicksOnDropdownMenuAndSelectSingleDeposit(List<String> amount) throws Exception {
        chooseSingleProduct1(amount.get(1));  // Select the 2nd Product from the Cell after Edit clicked
        ExtentCucumberAdapter.addTestStepScreenCaptureFromPath(Util.takeScreenshotasFile(driver));
    }

    public void userClicksOnDropdownMenuAndSelectSingleDepositAmount(List<String> productList) throws Exception {
        chooseSingleProduct1(productList.get(1));  // Select the 2nd Product from the Cell after Edit clicked
        ExtentCucumberAdapter.addTestStepScreenCaptureFromPath(Util.takeScreenshotasFile(driver));
    }


//    public void userClicksOnDropdownMenuAndSelectOne(List<String> product) throws Exception {
//            chooseSingleProduct(product.get(0), product.get(0));
//        ExtentCucumberAdapter.addTestStepScreenCaptureFromPath(Util.takeScreenshotasFile(driver));
//    }

    public void addIbanCloseBtn() throws Exception {
        if (webUtil.isElementclickable(addIbancloseslider, 20)) {
            webUtil.click(addIbancloseslider);
        } else {
            throw new ExceptionUtils("Close button x-path changed");
        }
    }
    ...
    public void cancelBtn() throws Exception {
        if (webUtil.isElementclickable(cancelbtnIban, 20)) {
            webUtil.click(cancelbtnIban);
        } else {
            throw new ExceptionUtils("Cancel button x-path changed");
        }
    }

    public void cancelBtnIban() throws Exception {
        if (webUtil.isElementclickable(cancelbutton, 20)) {
    ...
    public void allocate_fullamountandclickmodelconfrmbutton(int index) throws Exception {
        this.index = index;
            ...
            var product = data.getReinvestProducts().get(index - 1);
            var btnAllocateFullAmt =product.equals("Prize Bonds") ? By.xpath(String.format(allocatefullamount, index,"2")) : By.xpath(String.format(allocatefullamount, index,"1"));
            webUtil.click(btnAllocateFullAmt);
    }
