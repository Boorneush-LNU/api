...
public class Prize_Bond {
    ...
    private final By sectionEligible = By.id("sectionEligible");
    WebDriver driver;
    ...
    private final By reinvestOption = By.cssSelector("button.gtm-repayReinvest");
    ...
    public  String Content = "Prize Bond winnings payment optionn";
    ...
    private final By BlankAmountError = By.id("RFVPBAmount");
    private final String lessAmountErrorMessage = "Enter an amount or select 'Allocate full amount'.";
    private final By moreThanAvailableAmountError = By.id("CVPBAmount");
    ...
    private final By prizeBondRangeWindowError = By.id("pnlRangeError");
    ...
    private final By nearestEligibleAmount = By.id("nearest-amount");
    ...
    private final By ProfileAndSetting = By.xpath("//a[contains(text(),'Profile and Settings')]");
    ...
    private final By modalAddBankButton = By.xpath(".//button[@regstrnData-modal=\"add-bank-modal\"]");
    ...
    private final By AddBank = By.xpath("//button[@class='gta-cta button button--primary js-modalTrigger js-validate-button']");
...
public By HeaderContent = By.xpath("//h1[@id='prizeBondsTitle']");

    public By ParagraphSubHeader = By.xpath("//p[@id='NoprizeBondsTitle']");
    public By Paragraph = By.xpath("//p[@id='prizeBondsDescription2']");
    ...
    public void checkReinvestLink() throws Exception {
        if (webUtil.isElementVisible(reinvestOption, 20)) {
    ...
    public void enterAmount() {
        ...
        if (data.getChooseAmount().contentEquals("blank"))
            driver.findElement(enterAmount).sendKeys("");
        else {

            webUtil.sendKeys(enterAmount, data.getChooseAmount());
            System.out.println("Print the else value");
        }


    }

    public void error(String errorType, String expectedMessage) throws Exception {
        switch (errorType) {
            case "ChooseAmount-Blank":
                webUtil.sendKeys(enterAmount, "");
                webUtil.click(confirmBtn);
                webUtil.gettextlog(BlankAmountError, String::equals, expectedMessage);
                break;
            case "ChooseAmount-MoreThanAvailable":
                webUtil.sendKeys(enterAmount, "400000");
                webUtil.click(confirmBtn);
                webUtil.gettextlog(moreThanAvailableAmountError, String::equals, expectedMessage);
                break;
            case "BondRange-Checkbox":
                webUtil.gettextlog(prizeBondRangeWindowError, String::equals, expectedMessage);
                break;
        }
    ...
    public void clickConfirmButton() throws Exception {
        ...
        webUtil.click(confirmBtn);
        if(webUtil.isAllElementVisible(sectionEligible,10)) webUtil.click(confirmBtn);
//        if (webUtil.isElementVisible(nearestEligibleAmountLabel, 10)) {
//        if (Integer.parseInt(data.getChooseAmount()) % 6.25 != 0 || Integer.parseInt(data.getChooseAmount()) % 6.35 != 0) {
//
//                if (webUtil.getText(nearestEligibleAmountLabel).contentEquals("Showing nearest eligible amount:")) {
//                    int value = (int) (Integer.parseInt(data.getChooseAmount()) / 6.25);
//                    int value1 = (int) (Integer.parseInt(data.getChooseAmount()) / 6.35);
...
//        }
    ...
    public void Near1() throws InterruptedException {
          String Nearest =  webUtil.getText(Near);
        this.nearAmountText = Nearest;
        System.out.println("Stored Nearest Amount: " + this.nearAmountText);
    }
    ...
    public void verifyBondRangeWindow() {
        ...
        String expectedTooltipContent = data.getOptionType().equals("Cash-In") ? String.format(toottipContent, "repayment") : String.format(toottipContent, "reinvestment");
    ...
    public void clickProfileAndSettings() {
        String Profilesetting = "Profile and Settings";
        webUtil.gettextlog(ProfileAndSetting, String::equals, Profilesetting);
        webUtil.click(ProfileAndSetting);
    }
...
//        if(webUtil.isElementDisplayed(ibanModal,5))
    ...
    public static String getHoldingId(String responseBody,
                                      boolean repayable,
                                      boolean reinvestible) {

        List<Map<String, Object>> holdings =
                JsonPath.from(responseBody).getList("");

        return holdings.stream()
                .filter(h -> repayable == (Boolean) h.get("isRepayable"))
                .filter(h -> reinvestible == (Boolean) h.get("isReinvestible"))
                .map(h -> String.valueOf(h.get("holdingId")))
                .findFirst()
                .orElseThrow(() ->
                        new RuntimeException("Matching holding not found"));
    }
