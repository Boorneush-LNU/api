...
//    private final By cashinAmount = By.xpath("//div[contains(@class,'small-4 columns product-review-list__amount')]");
private final By Near = By.id("nearest-amount");
     ...
     * @throws Exception
     */
    public void reviewPagedetails(String ID, List<String> product, List<String> amount, String counter) throws Exception {

        getamount = String.join(",", amount);
        try {
...
//            webUtil.click(reviewConfirm);
    ...
    //Shan created this method. Usage : Validate only the Amount because prod won't be displayed in Review page for only cash In.
    public void reviewPagedetailsCashIn(String ID, List<String> amount, String counter) throws Exception {
        getamount = String.join(",", amount);
        try {
            webUtil.getWebDriverWait().until(ExpectedConditions.urlContains("review"));
        } catch (Exception e) {
...
//            webUtil.click(reviewTextVerify);
     ...
     * @param amount
     */
    public void reviewUtile(List<String> product, List<String> amount) {
    ...
    public void reviewUtileCashIn(List<String> amount) {
    ...
    public void reviewPBReinvest() throws Exception {
        reviewUtile(data.getReinvestProducts(), data.getReinvestAmounts());
    ...
    public void revutils(String counter) throws Exception {
        ...
        if ( data.getOptionType().equals("Reinvest-Repay")) {
            reviewCashing();
        } else if (data.getProduct().equals("Prize Bond")) {
            reviewCashinPB();
        }
    ...
    public Map<String, String> getMapforReview() {
        ...
        mapforReview.put("Deposit Account", "Deposit Account");
    ...
    public void bondRangeReviewPage() throws Exception {
        ...
        var IBAN = api.getUserDetails(data.getUsername()).get("bankAccountIban");
        ...
        if (Double.parseDouble(expectedAmnt) == prize_amount) {
            FrameworkLogger.log(LogType.EXTENT_AND_CONSOLE_PASS, "Prize Amount  matched: " + expectedAmnt + " with " + prize_amount);
        }


    }

    public void lightBondRangeReviewPage() throws Exception {
        webUtil.gettextlog(header, String::equals, "Transaction Review");
        webUtil.gettextlog(headerBelowMessage, String::equals, "Review your transaction details for Prize Bonds");
        webUtil.gettextlog(cashinIBANLabel, String::equals, "Cash in to bank account");
        webUtil.gettextlog(cashinTerms, String::equals, "All transactions are subject to Ireland State Savings Terms and Conditions.");

        log.info("Review Page Amount: " + webUtil.getText(cashinAmount).trim().replaceAll("[^0-9.]", ""));
...
//        if (data.getAllocateAllAmountIndex().equals("null")) {
//            Double liquid = Double.parseDouble(webUtil.getText(cashinAmount).replaceAll("[^0-9.]", ""));
//            double value = Double.parseDouble(data.getChooseAmount()) / 6.25;
//            double value1 = Double.parseDouble(data.getChooseAmount()) / 6.35;
//            double liquid1 = 6.25 * value;
////            double liquid2 = 6.35 * value1;
//            liquid1 = Math.round(liquid1 * 100.0) / 100.0;
//            System.out.println("Paaaisng1");
//            double liquid2 = 6.35 * value1;
//            liquid2 = Math.round(liquid2 * 100.0) / 100.0;
//            System.out.println("Paaaisng2");
//            if (liquid.toString().equals(String.valueOf(liquid1))) {
//                FrameworkLogger.log(LogType.EXTENT_AND_CONSOLE_PASS, "Amount matched" + liquid + " with amount entered: " + liquid1);
//            } else if (liquid.toString().equals(String.valueOf(liquid2))) {
//                FrameworkLogger.log(LogType.EXTENT_AND_CONSOLE_PASS, "Amount matched" + liquid + " with amount entered: " + liquid2);
//            } else {
...
//        }
        ...
        if (data.getAllocateAllAmountIndex().equals("null")) {
            Double actualLiquid = Double.parseDouble(webUtil.getText(cashinAmount).replaceAll("[^0-9.]", ""));
            double chooseAmount = Double.parseDouble(data.getChooseAmount());
            final double UnitPrice_1 = 6.25;
            final double UnitPrice_2 = 6.35;

            double units1 = Math.floor(chooseAmount / UnitPrice_1);
            double expectedLiquid1 = units1 * UnitPrice_1;
            expectedLiquid1 = Math.round(expectedLiquid1 * 100.0) / 100.0;

            double units2 = Math.floor(chooseAmount / UnitPrice_2);
            double expectedLiquid2 = units2 * UnitPrice_2;
            expectedLiquid2 = Math.round(expectedLiquid2 * 100.0) / 100.0;

            System.out.println("Input Amount (from Excel): " + chooseAmount);
            System.out.println("Expected 1 (6.25 rate): " + expectedLiquid1);
            System.out.println("Expected 2 (6.35 rate): " + expectedLiquid2);
            System.out.println("Actual UI Amount: " + actualLiquid);

            if (actualLiquid.equals(expectedLiquid1)) {
                FrameworkLogger.log(LogType.EXTENT_AND_CONSOLE_PASS,
                        "Amount matched. Actual: " + actualLiquid + " with Expected 1: " + expectedLiquid1);
            } else if (actualLiquid.equals(expectedLiquid2)) {
                FrameworkLogger.log(LogType.EXTENT_AND_CONSOLE_PASS,
                        "Amount matched. Actual: " + actualLiquid + " with Expected 2: " + expectedLiquid2);
            } else {
                throw new Exception("Amount not matched. Actual: " + actualLiquid +
                        " Expected were: " + expectedLiquid1 + " or " + expectedLiquid2);
            }
        } else {
            String liquid = (Modals.amount.replaceAll("[^0-9.]", ""));
            String liquid1 = webUtil.getText(cashinAmount).replaceAll("[^0-9.]", "");
    ...
    public void reviewAllocated() throws Exception {
        ...
        if (webUtil.isElementDisplayed(evaluateCashin1, 20)) {
            ...
            String maturityValue = Double.toString(holdings.getMaturityValue());
            if (!ramt.contains(maturityValue) || !ramt.contains(data.getReinvestAmounts().get(0))) {
    ...
    public void reviewAllocatedCashin() throws Exception {
        if (webUtil.isElementDisplayed(evaluateCashin1, 5)) {
            ...
            if (data.getOptionType().contentEquals("null")) {
                if (ramt.contentEquals(String.valueOf(avilableAmount))) {
                    log.info("Amount matched");
                } else if (Double.parseDouble(ramt) == Double.parseDouble(data.getReinvestAmounts().get(0))) {
                    log.info("Amount matched");
                } else {
                    throw new ExceptionUtils("Amount not matched on the review page");
                }
            } else {
                if (ramt.contains(String.valueOf(holdings.getMaturityValue())) || ramt.contentEquals(data.getReinvestAmounts().get(0))) {
                    log.info("Amount matched");
                } else {
                    throw new ExceptionUtils("Amount not matched on the review page");
                }
            }
            boolean isPrizeBond =data.getReinvestProducts().stream().anyMatch(s->s.contains("Prize Bond"));
            if (isPrizeBond) {
                try {
                    contentvalidation();
                } catch (Exception e) {
