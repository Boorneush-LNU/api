...
//    private final By enterMobilePrefix = By.xpath("//select[@id='ddl_PhonePrefixA']");
    private final By enterMobilePrefix = By.xpath("//select[@id='txtPhonePrefixA']");
//    private final By reEnterMobilePrefix = By.xpath("//input[@id='txtPhonePrefixB-selectized']");
private final By reEnterMobilePrefix = By.xpath("//select[@id='txtPhonePrefixB']");
    ...
    public void enterMobileNumber() throws Exception {
        if (webUtil.isElementDisplayed(enterMobileNumber, 10)) {
            try {
                if(REGISTRATION_DATA.getScenario().equals("Mobile_Number_Blank"))
                    REGISTRATION_DATA.setMobileNumber("");
                Thread.sleep(3000);
                    webUtil.sendKeys(enterMobileNumber, REGISTRATION_DATA.getMobileNumber());
                Thread.sleep(3000);
            } catch (Exception e) {
    ...
    public void reEnterMobileNumber() throws Exception {
        if (webUtil.isElementDisplayed(reEnterMobile, 10)) {
            try {
                if(REGISTRATION_DATA.getScenario().equals("Mobile_Number_Blank"))
                    REGISTRATION_DATA.setReEnterMobileNumber("");
                    webUtil.sendKeys(reEnterMobile, REGISTRATION_DATA.getReEnterMobileNumber());
            } catch (Exception e) {
    ...
    public void selectMobilePrefixMisMatch() throws Exception { //Shan Added
        String Prefix = "+353";
...
//                webUtil.selectDropDown(enterMobilePrefix, select -> select.selectByVisibleText(ExpeditedSteps.testData.get("prefix")));
                webUtil.selectDropDown(enterMobilePrefix, select -> select.selectByVisibleText(Prefix));
                Thread.sleep(3000);
    ...
    public void selectReMobilePrefixMisMatch() throws Exception { //Shan Added
        String RePrefix = "+964";
...
//                webUtil.sendKeys(reEnterMobilePrefix, (ExpeditedSteps.testData.get("Prefix re-enter")));
                webUtil.selectDropDown(reEnterMobilePrefix, select -> select.selectByVisibleText((RePrefix)));
                webUtil.click(By.xpath("//div[@class='medium-8 columns']"));
                Thread.sleep(2000);
            } catch (Exception e) {
    ...
    public void enterMobileNumberdetailsforTitle() throws Exception {
            ...
            try {
                webUtil.sendKeys(enterMobileNumber,  regstrnData.get("MobileNumber"));
                webUtil.sendKeys(reEnterMobile,  regstrnData.get("Re-EnterMobileNumber"));
