...
public final By emailNotReceived=By.xpath("//section[@class='dashboard-modal__content js-modal active']");
    //section[@data-name='resend-email']
//    public final By emailNotReceived=By.xpath("//section[@data-name='resend-email']");
    public final By mobileCodeNotReceived=By.id("sectionResend");
    ...
    public void enterSecurityCode() throws Exception {
        ...
        String indicator = REGISTRATION_DATA.getOtpIndicator();
        switch (indicator) {
            case "valid":
                enterOtp(otp);
                break;
    ...
    @SneakyThrows
    public void enterSecurityCodeforTitle(){
        webUtil.getWebDriverWait().until(ExpectedConditions.urlContains("expedited-mobile-security"));
        String otp = apiUtil.getOTP(otpUrl, 200, "0".concat(regstrnData.get("otpNumber")));
        webUtil.sendKeys(enterSecurityCode, otp);
    }

    public void enterOtp(String otp) throws InterruptedException {
        if (webUtil.isElementDisplayed(enterSecurityCode, 10)) {
            try {
                webUtil.sendKeys(enterSecurityCode, otp);
            } catch (Exception e) {
