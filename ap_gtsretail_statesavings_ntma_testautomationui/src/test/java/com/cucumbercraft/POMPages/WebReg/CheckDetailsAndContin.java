...
public class CheckDetailsAndContin {
    ...
    public void verifyAddressLine1() throws Exception {
        if (webUtil.isElementVisible(AddressLine1, 20)) {
            System.out.println(REGISTRATION_DATA.getAddressLine1());
            System.out.println(webUtil.getText(AddressLine1));
            if (!WordUtils.capitalizeFully(webUtil.getText(AddressLine1)).equals(REGISTRATION_DATA.getAddressLine1())) {
    ...
    public void verifyAddressLine2() throws Exception {
        if (!(REGISTRATION_DATA.getAddressLine2().equalsIgnoreCase("null"))) {
            if (webUtil.isElementDisplayed(AddressLine2, 20)) {
                if (!webUtil.getText(AddressLine2).equals(REGISTRATION_DATA.getAddressLine2())) {
    ...
    public void verifyCity() throws Exception {
        if (webUtil.isElementDisplayed(City, 20)) {
            ...
            System.out.println(REGISTRATION_DATA.getTown());
            if (!webUtil.getText(City).equals(REGISTRATION_DATA.getTown().toUpperCase())) {
    ...
    public void verifyCountry() throws Exception {
        if (webUtil.isElementDisplayed(Country, 20)) {
            webUtil.gettextlog(Country, String::equals, REGISTRATION_DATA.getCountryStateRegion(), "Country");
            if (!webUtil.getText(Country).equals(REGISTRATION_DATA.getCountryStateRegion())) {
    ...
    public void verifyEirCode() throws Exception {
        if (!(REGISTRATION_DATA.getEircode().equalsIgnoreCase("null"))) {
            if (webUtil.isElementDisplayed(eirCode, 20)) {
                if (!webUtil.getText(eirCode).equals(REGISTRATION_DATA.getEircode())) {
    ...
    public void verifyEmail() throws Exception {
        if (webUtil.isElementDisplayed(eMail, 20)) {
            if (!webUtil.getText(eMail).equals(REGISTRATION_DATA.getEmailAddress())) {
    ...
    public void verifyNumber() throws Exception {
        if (webUtil.isElementDisplayed(phoneNumber, 20)) {
            ...
            System.out.println(REGISTRATION_DATA.getPrefix().concat(REGISTRATION_DATA.getMobileNumber()));

            if (!webUtil.getText(phoneNumber).replaceAll(" ", "").contentEquals(REGISTRATION_DATA.getPrefix().concat(REGISTRATION_DATA.getMobileNumber()))) {
    ...
    public void clickConsent() throws Exception {
        ...
        WebElement element = webUtil.waitUntilElementLocated(consent,5);
    ...
    public void validateConsentError() {
        webUtil.gettextlog(By.id("check-box-consent-error"),String::equals,"Please accept the terms and conditions");
