...
public class AdditionalInformation {
    ...
    private final By btnContinueToPaymentIrish = By.xpath("//button[text()='Lean ar aghaidh chuig íocaíocht']");
    ...
    public AdditionalInformation manageCdeIrish(PurchaseModel data) throws Exception {
        ...
        if (!context.isOnlyPB() && !context.isContainsPBAndPBGift()) {
            handleCDEForNonPBIrish(data.getCDE(), context.isJoint());
        }
        return this;
    }
    ...
    private void handleCDEForNonPBIrish(List<String> cde, boolean isJoint) throws Exception {
        if (!isJoint && !CDE_PrimaryFlag) {
            handleSolePurchaseCDEIrish(cde);
        } else if (!CDE_PrimaryFlag && !CDE_SecondaryFlag) {
            handleJointPurchaseCDEIrish(cde);
        } else if (isJoint && !CDE_SecondaryFlag) {
            verifyPrimaryApplicantDetails();
            handleSecondaryApplicantCDEIrish(cde);
        } else if (isJoint && !CDE_PrimaryFlag) {
            handlePrimaryApplicantCDEIrish(cde);
            verifySecondaryApplicantDetails();
        } else {
            handleUnexpectedCDEState();
        }
    ...
    private void handleSolePurchaseCDEIrish(List<String> cde) throws Exception {
        waitForCDEPage();
        verifyCDEHeader("Faisnéis Bhreise");
        cdeDropdown(cde.get(0), cde.get(1), cde.get(2), cde.get(3), cde.get(4));
    ...
    private void handleJointPurchaseCDEIrish(List<String> cde) throws Exception {
        waitForCDEPage();
        verifyCDEHeader("Faisnéis Bhreise");
        verifyPrimaryApplicantDetails();
        verifySecondaryApplicantDetails();
    ...
    private void handlePrimaryApplicantCDEIrish(List<String> cde) throws Exception {
        waitForCDEPage();
        verifyCDEHeader("Faisnéis Bhreise");
    ...
    private void handleSecondaryApplicantCDEIrish(List<String> cde) throws Exception {
        waitForCDEPage();
        verifyCDEHeader("Faisnéis Bhreise");
    ...
    public AnpostPayment submitCDEdetailsIrish() {

        if (!context.isOnlyPB() && !context.isContainsPBAndPBGift()) {
            if (!context.isJoint() && !CDE_PrimaryFlag) {
                webUtil.clickLog(btnContinueToPaymentIrish, "Pay Button");
            } else if (!CDE_PrimaryFlag && !CDE_SecondaryFlag) {
                webUtil.clickLog(btnContinueToPaymentIrish, "Pay Button");
            } else if (context.isJoint() && !CDE_SecondaryFlag) {
                webUtil.clickLog(btnContinueToPaymentIrish, "Pay Button");
    ...
    public void errorCDE(String indicator,List<String> products,String journey) {
        if(!products.get(0).contains("Prize Bonds")){
            webUtil.clickLog(By.id("btnSubmitOrder"), "Submit");

            if (journey.contains("Joint")) {
                switch (indicator) {
                    case "N,N":
                        webUtil.scrollToView(By.id("RFVddlNationality"));
                        webUtil.gettextlog(By.id("RFVddlNationality"), String::equals, "Please provide nationality");
                        webUtil.gettextlog(By.id("RFVddlCountyOfBirth"), String::equals, "Please provide country of birth");
                        webUtil.gettextlog(By.id("RFVddlEmploymentStatus"), String::equals, "Please provide employment status");
                        webUtil.gettextlog(By.id("RFVddlIndustryType"), String::equals, "Please provide employment industry type");
                        webUtil.gettextlog(By.id("RFVnetIncome"), String::equals, "Please enter estimated monthly net income");
                        webUtil.scrollToView(By.id("RFVddlNationalitySecond"));
                        webUtil.gettextlog(By.id("RFVddlNationalitySecond"), String::equals, "Please provide nationality");
                        webUtil.gettextlog(By.id("RFVddlCountyOfBirthSecond"), String::equals, "Please provide country of birth");
                        webUtil.gettextlog(By.id("RFVddlEmploymentStatusSecond"), String::equals, "Please provide employment status");
                        webUtil.gettextlog(By.id("RFVddlIndustryTypeSecond"), String::equals, "Please provide employment industry type");
                        webUtil.gettextlog(By.id("RFVnetIncomeSecond"), String::equals, "Please enter estimated monthly net income");
                        break;
                    case "Y,N":
                        webUtil.scrollToView(By.id("RFVddlNationalitySecond"));
                        webUtil.gettextlog(By.id("RFVddlNationalitySecond"), String::equals, "Please provide nationality");
