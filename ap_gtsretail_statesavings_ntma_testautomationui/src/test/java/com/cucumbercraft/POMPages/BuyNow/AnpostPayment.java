...
public class AnpostPayment {
    ...
    private final By btnPay = By.id("btnPayNow");
    public String orderRefNo;
    private boolean modalNotVisible;
    private boolean tyHdrNotVisible;
    ...
    public AnpostPayment clickPayButton() {
        By tyheader = By.xpath("//h1[contains(text(),'Thank you')]");

        webUtil.scrollToView(btnPay);
        webUtil.click(btnPay);
        modalNotVisible = !webUtil.isElementVisible(modalProcessingLabel, 3);
        tyHdrNotVisible = !webUtil.isElementVisible(tyheader, 3);
        if (modalNotVisible && tyHdrNotVisible) {
            webUtil.click(btnPay);
        }
        return this;
    }
    ...
    public AnpostPayment waitForPaymentPage() {
        ...
        if (flag) {
            FrameworkLogger.log(LogType.EXTENT_AND_CONSOLE_PASS, "Payment page displayed");
    ...
    public AnpostPayment enterCardDetailsInfo(String HolderName, String cardNumber, String expiry, String cvv) {
        fillCardHolderName(HolderName);
    ...
    public ThankYou retryPaymentIfFailed(String cardNo, String expiry, String cvv) {
            ...
            if (webUtil.isElementVisible(modalErrorLabel, 5) && webUtil.getText(modalErrorLabel).equals("Error processing the transaction.")) {
                ...
                enterCardDetails(cardNo, expiry, cvv)
                        .clickCheckbox()
                        .clickPayButton();
